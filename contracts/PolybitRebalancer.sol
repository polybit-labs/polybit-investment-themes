...
contract PolybitRebalancer {
        ...
        returns (address[] memory, uint256[] memory, uint256[] memory)
    {
        ...
        if (adjustList.length > 0) {
            ...
            for (uint256 i = 0; i < adjustList.length; i++) {
                (, data.tokenBalanceInWeth) = IPolybitTheme(detfAddress)
    ...
    function calcTotalTargetBuyPercentage(
        ...
        data.totalBalance = IPolybitTheme(detfAddress).getTotalBalanceInWeth(
            ownedAssetsPrices
        );
    ...
    function createSellOrder(
        ...
        for (uint256 i = 0; i < sellList.length; i++) {
            (data.tokenBalance, data.tokenBalanceInWeth) = IPolybitTheme(
                detfAddress
    ...
    function createAdjustToBuyOrder(
        ...
        for (uint256 i = 0; i < adjustToBuyList.length; i++) {
            if (adjustToBuyList[i] != address(0)) {
                uint256 decimals = IERC20(adjustToBuyList[i]).decimals();
                ...
                data.precisionAmountIn =
                    ...
                    10 ** 8;
                data.amountOut =
                    (10 ** decimals * data.precisionAmountIn) /
                    data.tokenPrice;
    ...
    function createBuyOrder(
        ...
        uint256 index = 0;
        for (uint256 i = 0; i < buyList.length; i++) {
            uint256 decimals = IERC20(buyList[i]).decimals();
